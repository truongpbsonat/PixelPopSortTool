...
def test_dragging_right_edge_resizes_and_records_one_change(qtbot):
    ...
    end = editor.mapFromScene(QPointF(4 * CELL, CELL))
...
def test_dragging_bottom_right_corner_resizes_both_dimensions(qtbot):
    ...
    end = editor.mapFromScene(QPointF(CELL, 2 * CELL))
...
def test_shrinking_then_expanding_in_one_drag_does_not_discard_pixels(qtbot):
    ...
    qtbot.mousePress(editor.viewport(), Qt.LeftButton, pos=start)
    qtbot.mouseMove(editor.viewport(), pos=editor.mapFromScene(QPointF(CELL, CELL / 2)))
    assert editor.level.pixel_grid.color_ids == [0]
    qtbot.mouseMove(editor.viewport(), pos=editor.mapFromScene(QPointF(3 * CELL, CELL / 2)))
    qtbot.mouseRelease(
        editor.viewport(),
        Qt.LeftButton,
        pos=editor.mapFromScene(QPointF(3 * CELL, CELL / 2)),
    )
